...
- Upload the packaged Doom `.h3img` image over the separate UART connection, or stage it as `DOOM.IMG` on micro-SD.
...
60 MHz LiteDRAM user clock. The complete board build creates the matching
monitor, FPGA bitstream, and Doom image under the board-scoped build paths:
...
The current default routing checkpoint is seed 83 with HeAP
`timingweight=30`. Treat that as a regression checkpoint for the current RTL,
toolchain, and generated LiteDRAM files rather than a portable guarantee. Any
complete rebuild that materially changes the netlist or tool flow should be
rerouted and requalified on hardware.
...
Routing time is taken into account when choosing default seeds.
...
Enter DFU by removing power, holding PCB `BTN3`, and reconnecting the ULX4M
Micro-B USB cable. After VID:PID `1d50:614b` appears, release `BTN3`; it does
not need to remain held during programming. If WSL reports `Permission denied`
for either bundled Windows executable, restore its executable bit first.
...
Then load the 40 MHz monitor in another terminal. The board-specific GDB
command file deliberately selects the monitor generated by the complete ULX4M-LD
build:

```bash
./bin/gdb/riscv-none-elf-gdb.exe -batch \
    -x scripts/gdb/load-ulx4m-ld-85f-monitor.gdb
```
...
Load the **board-specific monitor built with the same clock/memory profile as the
FPGA image**. OpenOCD must already be running. Do not use an old generic or test
ELF from `./bin`; a stale monitor can run well enough to blink LEDs while using
the wrong UART divisor or an obsolete Doom-image protocol.

The repository now provides one GDB command file for each supported FPGA target:
...
Each helper selects the monitor produced by the corresponding complete board
build:
...
From WSL/Bash, for ULX3S 85F:

```bash
./bin/gdb/riscv-none-elf-gdb.exe -batch \
    -x scripts/gdb/load-ulx3s-85f-monitor.gdb
```
...
```bash
./bin/gdb/riscv-none-elf-gdb.exe -batch \
    -x scripts/gdb/load-ulx3s-12f-monitor.gdb
```

When entering equivalent commands manually, remember that WSL is still Bash:
use forward-slash paths and `\` line continuation. Windows Command Prompt uses
backslash paths and `^` continuation. Do not paste `cmd.exe` syntax directly into
Bash.

The shell loader remains available when you intentionally want to name an ELF
explicitly:
...
After a successful `load` and `compare-sections`, the monitor should respond on
its board UART at the configured 115200 baud and accept Doom image/IWAD uploads.
...
For ULX4M-LD, use `build/ulx4m-ld/doom-image/hazard3-doom.h3img` and the Tigard
VCP UART port.
...
The UART uploader sends the monitor's load command and waits for an `H3L READY`
reply before transmitting the payload. A timeout therefore means the expected
monitor did not answer on that UART at that baud; it does **not** mean the Doom
payload itself failed CRC validation.

Use the board-specific `hazard3-boot-monitor.elf` produced by the same build as
the FPGA/bitstream instead of an old generic/test ELF. For example, ULX4M-LD
uses:
...
A stale monitor can be particularly misleading: LEDs may blink even though the
monitor was built for a different system clock or an older image-header format.
For a 40 MHz ULX4M-LD image, normal communication is still **115200 baud**. If an
old monitor only responds near 92160 baud, or responds with `H3L ERROR invalid
header` to a newly generated `.h3img`, replace it with the current board-specific
monitor rather than treating 92160 as the supported baud rate.
...
$ ./doom/upload-doom-image.py \
    ...
    --port /dev/ttyS8
Opening /dev/ttyS8 at 115200; payload=590944, CRC32=0x6cb6e796
error: timed out waiting for loader response
```

Verify OpenOCD/JTAG, load the current monitor, confirm `compare-sections`, then
retry the UART upload at 115200.
...
ELF: "C:\temp\Hazard3-Doom\build\ulx3s\monitor\hazard3-boot-monitor.elf"
